...
struct CircularLayoutMenu: View {
    let startAnglePercentage: CGFloat
    let radiusGapPercentage: CGFloat
    let zOrder: ZOrder
    let itemSizing: RelativeSizing
    let itemCount: Int

    let onItemCountChange: (Int) -> Void
    let onStartAngleChange: (CGFloat) -> Void
    let onRadiusGapChange: (CGFloat) -> Void
    ...
    @State private var radiusGap: Double
    ...
    init(
        startAnglePercentage: CGFloat,
        radiusGapPercentage: CGFloat,
        zOrder: ZOrder,
        itemSizing: RelativeSizing,
        itemCount: Int,
        onItemCountChange: @escaping (Int) -> Void,
        onStartAngleChange: @escaping (CGFloat) -> Void,
        onRadiusGapChange: @escaping (CGFloat) -> Void,
        ...
        self.onStartAngleChange = onStartAngleChange
        self.onRadiusGapChange = onRadiusGapChange
        ...
        self._startAngle = State(initialValue: Double(startAnglePercentage))
        self._radiusGap = State(initialValue: Double(radiusGapPercentage))
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack {
                    ...
                    Slider(value: $startAngle, in: 0.0...1.0, step: 0.05)
                        .onChange(of: startAngle) { _, newValue in
                            onStartAngleChange(CGFloat(newValue))
                        }
                ...
                Text(String(format: "%.2f", startAngle))
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                HStack {
                    ...
                    Slider(value: $radiusGap, in: 0.0...3.0, step: 0.1)
                        .onChange(of: radiusGap) { _, newValue in
                            onRadiusGapChange(CGFloat(newValue))
                        }
                ...
                Text(String(format: "%.1f", radiusGap))
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
